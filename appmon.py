...
app_name, platform, script_path, list_apps, output_dir, spawn = [0] * 6
...
@app.get("/api/fetch", response_class=ORJSONResponse)
...
@app.get("/monitor/",)
def monitor_page(request: Request, app):

    return templates.TemplateResponse(
        "monitor.html", dict(request=request, app_name=app)
    )
...
def init_opts():
    ...
    parser.add_argument(
        ...
        dest="no_hook",
        default="",
        help='start server without hook, aka. view the data history',
    )
    parser.add_argument(
        "--spawn",
        action="store",
    ...
    parser.add_argument(
        "-v", action="version", version="AppMon Sniffer v0.1, Nishant Das Patnaik, 2016"
    )

    

    global output_dir, report_name
    global app_name, platform, script_path, list_apps, output_dir, spawn

    args = parser.parse_args()
    app_name = args.app_name
    platform = args.platform
    script_path = args.script_path
    list_apps = int(args.list_apps)
    spawn = int(args.spawn)

    output_dir = args.output_dir if args.output_dir else os.path.join(".", "app_dumps")

    report_name = args.report if args.report else app_name
        ...
        if platform == "ios" or platform == "android":
            try:
                ...
                if platform == "android":
                    ...
                    print(
                        colored(
                            "HINT: Is `frida-server` running on mobile device (with +x permissions)?",
                            "blue",
                        )
                    )
                    ...
                    print(
                        colored(
                            "HINT: Have you installed `frida` module from Cydia?",
                            "blue",
                        )
                    )
                    print(
                        colored(
                            "HINT: Have used `ipa_installer` to inject the `FridaGadget` shared lbrary?",
                            "blue",
                        )
                    )
                    sys.exit(1)
            ...
            try:
                ...
                print(
                    colored(
                        "HINT: Have you successfully integrated the FridaGadget dylib with the XCode Project?",
                        "blue",
                    )
                )
                print(
